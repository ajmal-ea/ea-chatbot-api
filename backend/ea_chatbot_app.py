...
# Add a new model for meeting scheduler configuration
...
async def create_hubspot_contact(contact: HubSpotContactRequest, session_id: Optional[str] = None):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            timestamp_ms = int(datetime.now().timestamp() * 1000)  # Convert to milliseconds
            
            # Create a note for the form message if provided
            if contact.note:
                note_payload = {
                    "properties": {
                ...
                if note_response.status_code == 201:
                    logger.info(f"Successfully created form note for contact: {contact_id}")
                else:
...
async def metrics():
    ...
    try:
        ACTIVE_SESSIONS.set(await get_active_sessions_count())  # Update active sessions on metrics scrape
...
async def get_meeting_config():
    ...
    This endpoint returns the configuration for the meeting scheduler,
    including the URL, provider, and other details.
    """
    # In a production environment, you might want to load this from a database
    # or environment variables instead of hardcoding it
...
if __name__ == "__main__":
    ...
    print(f"MISTRAL_API_KEY: {os.getenv('MISTRALAI_API_KEY')}")
